...
class ReadingProducts(View):
    def get(self, request):
        ...
        return render(request, 'myapp4/all_products.html', {'products': products})
    ...
    if request.method == 'POST':
        ...
        if form.has_changed():
            if form.is_valid():
                ...
                return render(request, 'myapp4/update_product_completed.html')
        logger.info(f'The product data has not changed')
        return render(request, 'myapp4/update_product_not_changed.html')
    else:
        # открываем начальную форму с исходными данными товара
        form = UpdateProduct(initial=product_data)
    return render(request, 'myapp4/update_product.html', {'form': form})
...
def show_image(request, product_id):
    ...
    if request.method == 'POST':
        ...
        form = UpdateImage()
    return render(request, 'myapp4/show_image.html', {'product': product, 'form': form})